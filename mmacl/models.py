...
from mmacl.layers import ProposedConv


class HyperEncoder(nn.Module):
    def __init__(self, alpha, in_dim, edge_dim, node_dim, num_layers=1, act: Callable = nn.PReLU()):
        ...
        if num_layers == 1:
            self.convs.append(ProposedConv(self.transfer, self.alpha, self.in_dim, self.edge_dim, self.node_dim, cached=False, act=act))
        else:
            self.convs.append(ProposedConv(self.transfer, self.alpha, self.in_dim, self.edge_dim, self.node_dim, cached=False, act=act))
            for _ in range(self.num_layers - 2):
                self.convs.append(ProposedConv(self.transfer, self.alpha, self.node_dim, self.edge_dim, self.node_dim, cached=False, act=act))
            self.convs.append(ProposedConv(self.transfer, self.alpha, self.node_dim, self.edge_dim, self.node_dim, cached=False, act=act))
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, num_nodes: int, num_edges: int):
        for i in range(self.num_layers):
            x, x_g = self.convs[i](x, hyperedge_index, num_nodes, num_edges)
            # pre 빼는 거 확인?
            # x = self.act(x)
        return x, x_g # act, act



class MMACL(nn.Module):
    def __init__(self, encoder: HyperEncoder, proj_dim: int, feature: int,):
        super(MMACL, self).__init__()
        ...
        # fc_dim = feature + self.node_dim
        fc_dim = self.node_dim
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        n, n_g = self.encoder(x, self_loop_hyperedge_index, num_nodes, num_edges + num_nodes)
        return n, n_g
